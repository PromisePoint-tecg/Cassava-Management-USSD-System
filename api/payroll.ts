...
export const getAllPayrolls = async (params?: {
  ...
  const response: any = await apiClient.get<PaginatedPayrollResponse>(
  ...
  const normalizePayroll = (p: any): Payroll => ({
    ...
    processedStaffCount:
      p.processedStaffCount || p.processed_count || p.processedCount || 0,
    failedStaffCount:
      p.failedStaffCount || p.failed_count || p.failedCount || 0,
    // Financial amounts
    ...
    totalPensionEmployee:
      p.totalPensionEmployee || p.total_pension_employee || 0,
    totalPensionEmployer:
      p.totalPensionEmployer || p.total_pension_employer || 0,
    totalPensionAmount:
      p.totalPensionAmount ||
      (p.total_pension_employee || 0) + (p.total_pension_employer || 0) ||
      (p.totalPensionEmployee || 0) + (p.totalPensionEmployer || 0),
    totalTaxDeducted: p.totalTaxDeducted || p.total_tax_deducted || 0,
    totalTaxAmount:
      p.totalTaxAmount || p.total_tax_deducted || p.totalTaxDeducted || 0,
    // Metadata
...
export const processPayroll = async (
  ...
  const response: any = await apiClient.post<ProcessPayrollResponse>(
...
export const getPayrollTransactions = async (
  ...
  const normalizeTransaction = (t: any): PayrollTransaction => ({
    ...
    staffName:
      t.staffName ||
      t.staff_name ||
      `${t.staff_id?.first_name || ""} ${t.staff_id?.last_name || ""}`.trim(),
    staffFullName:
      t.staffFullName ||
      t.staff_name ||
      `${t.staff_id?.first_name || ""} ${t.staff_id?.last_name || ""}`.trim() ||
      "Unknown",
    staffEmployeeId:
      t.staffEmployeeId || t.employee_id || t.staff_id?.employee_id || "N/A",
    ...
    pensionEmployeeContribution:
      t.pensionEmployeeContribution || t.pension_employee_contribution || 0,
    pensionEmployerContribution:
      t.pensionEmployerContribution || t.pension_employer_contribution || 0,
    employeePensionContribution:
      t.employeePensionContribution ||
      t.pension_employee_contribution ||
      t.pensionEmployeeContribution ||
      0,
    employerPensionContribution:
      t.employerPensionContribution ||
      t.pension_employer_contribution ||
      t.pensionEmployerContribution ||
      0,
    totalPensionContribution:
      t.totalPensionContribution || t.total_pension_contribution || 0,
    ...
    savingsDeduction: t.savingsDeduction || t.savings_deduction || 0,
    creditedSalary: t.creditedSalary || t.credited_salary || 0,
    ...
    payrollPeriodLabel:
      t.payrollPeriodLabel || t.payroll_id?.period_label || "N/A",
...
export const getStaffPayrollHistory = async (
  ...
  const normalizeTransaction = (t: any): PayrollTransaction => {
    ...
    return {
      ...
      payrollId:
        t.payrollId ||
        (typeof t.payroll_id === "object" ? t.payroll_id?._id : t.payroll_id),
...
export const retryFailedTransaction = async (
  ...
  const response: any = await apiClient.post(
    `/payroll/transaction/${transactionId}/retry`
  );
  return response.data || response;
};
...
export const getPayrollStatistics = async (
  ...
  const response: any = await apiClient.get<PayrollStatistics>(
    `/payroll/${payrollId}/statistics`
  );
  return response.data || response;
};
 ...
 */
export const getAllPayrollTransactions = async (params?: {
  page?: number;
  limit?: number;
}): Promise<PaginatedTransactionResponse> => {
  const queryParams = new URLSearchParams();
  ...
  const response: any = await apiClient.get(
    `/payroll/transactions?${queryParams.toString()}`
  );
  const rawData = response.data?.data || response.data || response;
  ...
  const normalizeTransaction = (t: any): PayrollTransaction => ({
    id: t._id || t.id,
    payrollId: t.payroll_id?._id || t.payroll_id || t.payrollId,
    staffId: t.staff_id?._id || t.staff_id || t.staffId,
    userId: t.user_id || t.userId,
    employeeId: t.employee_id || t.employeeId,
    staffName: t.staff_name || t.staffName,
    staffFullName: t.staff_name || t.staffName,
    staffEmployeeId: t.employee_id || t.employeeId,
    department: t.department,
    role: t.role,
    grossSalary: t.gross_salary || t.grossSalary,
    pensionEmployeeContribution:
      t.pension_employee_contribution || t.pensionEmployeeContribution,
    pensionEmployerContribution:
      t.pension_employer_contribution || t.pensionEmployerContribution,
    employeePensionContribution:
      t.pension_employee_contribution || t.pensionEmployeeContribution,
    employerPensionContribution:
      t.pension_employer_contribution || t.pensionEmployerContribution,
    totalPensionContribution:
      t.total_pension_contribution || t.totalPensionContribution,
    taxDeduction: t.tax_deduction || t.taxDeduction,
    otherDeductions: t.other_deductions || t.otherDeductions,
    totalDeductions: t.total_deductions || t.totalDeductions,
    netSalary: t.net_salary || t.netSalary,
    savingsDeduction: t.savings_deduction || t.savingsDeduction || 0,
    creditedSalary: t.credited_salary || t.creditedSalary,
    status: t.status || "pending",
    paymentStatus: t.status || "pending",
    paymentReference: t.payment_reference || t.paymentReference,
    paidAt: t.paid_at || t.paidAt,
    paymentDate: t.paid_at || t.paidAt,
    payrollPeriodLabel:
      t.payroll_id?.period_label || t.payroll_id?.periodLabel || "N/A",
    failedReason: t.failed_reason || t.failedReason,
 ...
 */
export const createOrganizationWallet = async (
  organizationName: string
): Promise<any> => {
  const response: any = await apiClient.post(
    "/admin/wallet/organization/create",
